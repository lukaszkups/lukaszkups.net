...
export default {
  ...
  getListEntries: (_path: string, listObj: any) => {
    return new Promise((resolve, reject) => {
  ...
  getMdFileContents: (_path: string, _index: number, _parentOutputPath: string, parentObj: any) => {
    return new Promise((resolve, reject) => {
  ...
  getAllContent: (_store: any) => {
    return module.exports.getPages(_store).then(() => {
      let promises: any[] = []
      // get all lists file contents
      _store.lists.map((list: any, index: number) => {
        promises.push(module.exports.getMdFileContents(list.path, index).then((obj: any)=> {
          _store.lists[index] = {..._store.lists[index], ...obj}
        }))
        promises.push(module.exports.getListEntries(list.entriesPath, list).then((arr: any[]) => {
      ...
      _store.pages.map((page: any, index: number) => {
        promises.push(module.exports.getMdFileContents(page.path, index).then((obj: any) => {
      ...
      return Promise.all(promises).then(() => {
        ...
        _store.lists.map((list: any, index: number) => {
          listPromises.push(module.exports.getListEntries(list.entriesPath, list).then((arr: any[]) => {
            _store.lists[index].entries = arr
            return Promise.resolve()
          }))
        })
  ...
  createOutputFolders: (_store: any): Promise<void> => {
    let promises: any[] = []
    ...
    _store.lists.map((list: any) => {
      promises.push(new Promise((resolve: (value: void), reject) => {
        try {
